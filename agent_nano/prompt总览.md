...
- [phase_a.py](/f:/workspace/tutu内容/agent_nano/phase_a.py)
- [phase_b_nano_banana_images.py](/f:/workspace/tutu内容/agent_nano/phase_b_nano_banana_images.py)
- [phase_c_seedance_i2v_prompts.py](/f:/workspace/tutu内容/agent_nano/phase_c_seedance_i2v_prompts.py)
- [phase_d_seedance_videos.py](/f:/workspace/tutu内容/agent_nano/phase_d_seedance_videos.py)
...
这一层不写进 [phase_a.py](/f:/workspace/tutu内容/agent_nano/phase_a.py)，因为它依赖真实首帧图。系统提示词单独放在：
...
- 只用首帧真实可见的场景、道具和空间关系
- A2 的核心动作只作为视频事件的起点、动机或主体线索，不限制整段视频只重复同一个动作
- 不要让蘑菇TUTU只围绕起始物体做单纯移动、推拉或观察
- 后续动作应从首帧真实可见的物体、边界、材质、光线、空间层次和环境关系里继续发散
- 让视频形成“起始动作 → 情境展开 → 多步互动 → 明确结果”的连续小故事
- 丰富故事不等于引入新场景、新角色或首帧不存在的重要道具
- 按 15 秒视频时长组织动作节奏，有起始、推进、轻微变化和收束
- 每条 prompt 至少包含 2-3 个清楚可见的动作阶段，不能只有一次触碰或一次观察
- 动作中要体现蘑菇TUTU的身体重心变化，例如前倾、后仰、蹲下、撑住、踮脚、转身、扶住物体、后退调整、再次发力
- 与可见物体互动时，物体要产生合理、连续、可见的反馈，例如滑动、晃动、翻起、轻微旋转、弹回或停住
- 环境、构图、光照和背景物体保持稳定，但蘑菇TUTU的动作要清楚、有幅度、可见
- 动作过程中自然包含表情或眼神变化，而且要随动作推进产生阶段变化，不要写成固定模板
- 镜头不必全程固定稳定观察，可以根据动作阶段进行自然切换、景别变化和平滑跟随
- 15 秒内允许 1-2 次合理镜头切换，例如中景动作、侧向发力、俯视物体反馈、再回到主体完成动作
- 每条 prompt 必须明确写出镜头过程，不能只在结尾补一句“镜头稳定跟随”
- 镜头过程建议写成“镜头先……随后切到……接着……最后……”，至少包含两个不同视角或景别
- 镜头切换必须顺着动作发生，不要跳脱首帧空间结构，不要快速乱切，不要把重点写成“极轻微运镜”
- 不要频繁使用“极轻微”“微幅”“小范围”“几毫米”“几乎不动”“安静收住”“轻轻停住”“轻轻”“缓慢”“安静”“微微”“小心翼翼”等削弱动作幅度的表达
- 不要写回静态文生图 prompt
- 不要拉近镜头
...
在 `agent_nano` 版本里，Phase B 文生图也会默认带上 `reference.png`：`text_to_image_prompt + reference.png -> Nano Banana 首帧图`。这一步用参考图稳定蘑菇TUTU的角色外观；Phase C 仍然只读取首帧图和 A1/A2 信息来写视频 prompt；Phase D 再把首帧图和 `reference.png` 一起提交给 Seedance。

Phase B 发给 Nano Banana 的默认补充规则还包括：生成 `9:16` 竖版首帧图；小蘑菇TUTU的画面占比不得超过整体画面大小的五分之一；场景中不要出现尘埃，不要出现任何发光的粒子状，光线要柔和，不要有光点或圆形光斑；小蘑菇TUTU位置要符合物理规律，禁止悬浮、物理穿模等。
