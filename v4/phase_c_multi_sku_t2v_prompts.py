...
def call_claude(system_prompt: str, user_prompt: str, timeout: int = 600) -> str:
    """用 converse-stream 突破 API Gateway 29s 超时；http.client 保留 header 大小写。"""
    import http.client
    from urllib.parse import urlparse

    if not CLAUDE_JWT_TOKEN:
        raise RuntimeError("缺少环境变量 CLAUDE_JWT_TOKEN")
    full_url = f"{CLAUDE_API_BASE}/model/{CLAUDE_MODEL_ID}/converse-stream"
    parsed = urlparse(full_url)
    payload = {
        "system": [{"text": system_prompt}],
        "messages": [{"role": "user", "content": [{"text": user_prompt}]}],
        "inferenceConfig": {"temperature": 0.85, "maxTokens": 8000},
    }
    ...
    try:
        ...
        try:
            ...
            print(f"[claude-stream] IncompleteRead, 用部分 {len(resp_body)}B", flush=True)
        if resp.status != 200:
    ...
    if not text_parts:
        raise RuntimeError("Claude stream 没拿到任何 text delta")
...
def run(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    todo = [bp for bp in selected if bp["context_id"] not in by_context_id or args.force]
    for bp in selected:
        if bp["context_id"] in by_context_id and not args.force:
            print(f"[skip] {int(bp['index']):02d} {bp['context_id']}")

    def _process_one(blueprint: dict[str, Any]) -> tuple[dict[str, Any], Exception | None]:
        context_id = blueprint["context_id"]
        index = int(blueprint["index"])
        title = blueprint.get("title", "")
        sku_index = blueprint.get("sku_index")
        print(f"[run] {index:02d} {context_id} sku={sku_index} {title}", flush=True)
        prompt = ""
        last_error: Exception | None = None
        for attempt in range(1, args.retries + 2):
            try:
                user_prompt_text = build_user_prompt(blueprint)
                if CLAUDE_JWT_TOKEN:
                    raw = call_claude(system_prompt, user_prompt_text, timeout=args.timeout)
                else:
                    raw = call_gemini(system_prompt, user_prompt_text, timeout=args.timeout)
                ...
                last_error = exc
                print(f"[retry] {index:02d} attempt {attempt} failed: {exc}", flush=True)
                if attempt <= args.retries:
                    time.sleep(args.retry_sleep)
        if not prompt:
            return ({"context_id": context_id, "index": index}, last_error)
        return ({
            "index": index,
    ...
    if todo:
        ...
        n_workers = int(os.environ.get("PHASE_C_WORKERS", "10"))
        n_workers = max(1, min(n_workers, len(todo)))
        print(f"[phase-c] processing {len(todo)} blueprints in parallel (workers={n_workers})", flush=True)
        write_lock = __import__("threading").Lock()
        with ThreadPoolExecutor(max_workers=n_workers) as pool:
            futures = {pool.submit(_process_one, bp): bp for bp in todo}
            completed = 0
            for fut in as_completed(futures):
                rec, err = fut.result()
                if err is not None:
                    print(f"[phase-c-FAIL] {rec.get('context_id')}: {err}", flush=True)
                    continue
                with write_lock:
                    by_context_id[rec["context_id"]] = rec
                ...
                if completed % 5 == 0 or completed == len(todo):
                    print(f"[phase-c] {completed}/{len(todo)} done", flush=True)
