...
class RunArtifacts:
    ...
    output_dir: Path
    contexts_path: Path
    text_to_image_prompts_path: Path
    manifest_path: Path
...
def build_event_id(context: dict[str, Any]) -> str:
    slot = str(context.get("slot", "slot")).replace(" ", "_")
    return f"evt_{context['context_id']}_{slot}"
...
def build_event_generation_prompts(context_batch: list[dict[str, Any]]) -> tuple[str, str]:
    personality = load_text(PERSONALITY_PATH)
    constitution = load_text(CONSTITUTION_PATH)
...
- 即使 context 里出现“微小”“微缩”“体量”“比例”“尺度”等词，text_to_image_prompt 也不要照抄这些词
...
"""
    ...
    user_prompt = f"""下面是一批已经做完 A1 输入融合的上下文，请为每条上下文各生成 1 条文生图 prompt。
...
[
  {{
    ...
    "triggered_by": "action",
    "text_to_image_prompt": "蘑菇TUTU在柔软蓬松的枕头边缘，用两只手把一角纸巾垫到枕面凹陷处，安静明亮的室内生活氛围，画面干净通透，柔和的光束，奶油般柔和的散景"
  }}
]
```
...
上下文列表：
```json
{json.dumps(context_batch, ensure_ascii=False, indent=2)}
```
"""
    return system_prompt, user_prompt


def generate_events_with_gemini(contexts: list[dict[str, Any]], batch_size: int) -> list[dict[str, Any]]:
    all_events: list[dict[str, Any]] = []
    for batch in chunks(contexts, batch_size):
        system_prompt, user_prompt = build_event_generation_prompts(batch)
        raw = call_gemini(system_prompt, user_prompt)
        parsed = parse_json_block(raw, default=[])
        if not isinstance(parsed, list):
            raise ValueError("Gemini 返回的 event 结果不是 JSON 数组。")

        by_context = {item["context_id"]: item for item in batch}
        batch_events: list[dict[str, Any]] = []
        for idx, item in enumerate(parsed, start=1):
            context = by_context.get(item.get("context_id", ""))
            if not context:
                continue
            event = {
                "event_id": build_event_id(context),
                "context_id": context["context_id"],
                "slot": item.get("slot") or context["slot"],
                "slot_time_hint": context["slot_time_hint"],
                "title": str(item.get("title", "")).strip(),
                "summary": str(item.get("summary", "")).strip(),
                "triggered_by": item.get("triggered_by") or infer_trigger(context),
                "text_to_image_prompt": str(item.get("text_to_image_prompt", "")).strip(),
            }
            if not event["text_to_image_prompt"]:
                event["text_to_image_prompt"] = fallback_text_to_image_prompt_from_context(
                    context,
                    event["title"],
                    event["summary"],
                )
            try:
                ...
                event = fallback_event_from_context(context)
                validate_event(event)
            batch_events.append(event)

        returned_context_ids = {evt["context_id"] for evt in batch_events}
        if len(returned_context_ids) < len(by_context):
            missing = [ctx for ctx in batch if ctx["context_id"] not in returned_context_ids]
            for context in missing:
                fallback_event = fallback_event_from_context(context)
                validate_event(fallback_event)
                batch_events.append(fallback_event)
...
def infer_trigger(context: dict[str, Any]) -> str:
    for source in context.get("trigger_priority", []):
        if source == "action" and context.get("action_theme"):
            return "action"
        if source == "mood" and context.get("mood_theme"):
            return "mood"
        if source == "weather" and context.get("weather"):
            return "weather"
        if source == "background" and (context.get("background") or context.get("background_env")):
            return "background"
        if source == "lifestyle" and context.get("lifestyle_theme"):
            return "lifestyle"
        if source == "daily" and context.get("daily"):
            return "daily"
    return "daily"


def fallback_event_from_context(context: dict[str, Any]) -> dict[str, Any]:
    triggered_by = infer_trigger(context)
    if triggered_by == "action":
        title = "小物互动"
        summary = f"蘑菇TUTU围绕“{context.get('action_theme', '轻动作')}”完成一个轻巧、可视化的互动场景。"
    elif triggered_by == "mood":
        title = "轻轻整理"
        summary = f"蘑菇TUTU带着“{context.get('mood_theme', '小安静')}”的底色，在当前环境里整理或拨动身边的小物。"
    elif triggered_by == "weather" and "雨" in context.get("weather", ""):
        title = "雨痕小河"
        summary = "蘑菇TUTU用小纸片轻轻拨开窗边雨痕，让水痕变成一条弯弯的小河。"
    elif triggered_by == "lifestyle":
        title = "顺手整理"
        summary = f"蘑菇TUTU顺着“{context.get('lifestyle_theme', '自然生活感')}”的气质，和身边日常小物完成一个轻巧、连贯的小互动。"
    elif triggered_by == "background":
        title = "借物做事"
        summary = f"蘑菇TUTU围绕“{context.get('background') or context.get('background_topic') or '当前背景'}”借用身边小物完成一个轻巧、可视化的小动作。"
    else:
        title = "日常小动作"
        summary = "蘑菇TUTU按自己的节奏和身边日常小物互动，做了一个轻微、具体、适合拍成首帧图的小动作。"
    ...
    return {
        ...
        "text_to_image_prompt": fallback_text_to_image_prompt_from_context(context, title, summary),
    }


def strip_size_words(text: Any) -> str:
    cleaned = str(text or "")
    replacements = [
        ("4cm 高的微缩有生命蘑菇", ""),
        ("微缩有生命蘑菇", ""),
...
def fallback_text_to_image_prompt_from_context(context: dict[str, Any], title: str, summary: str) -> str:
    title = strip_size_words(title)
    summary = strip_size_words(summary)
    background = strip_size_words(context.get("background") or context.get("background_env") or "干净通透的生活场景")
    action_theme = strip_size_words(context.get("action_theme", "轻巧互动"))
    lifestyle_theme = strip_size_words(context.get("lifestyle_theme", "自然生活感"))
    mood_theme = strip_size_words(context.get("mood_theme", "轻微情绪"))
    return (
        "蘑菇TUTU，"
        f"出现在{background}中，"
        f"画面主题是“{title}”，{summary}"
        f"它正在围绕身边日常小物完成“{action_theme}”，"
        f"整体呈现{lifestyle_theme}，"
        f"情绪是{mood_theme}，"
        "画面干净通透，明亮均匀的光线，奶油般柔和的散景。"
    )


def validate_event(event: dict[str, Any]) -> None:
    required = ["context_id", "slot", "title", "summary", "triggered_by", "text_to_image_prompt"]
    for key in required:
        if key not in event:
            raise ValueError(f"event 缺字段: {key}")
    if event["triggered_by"] not in {"daily", "weather", "background", "lifestyle", "action", "mood"}:
        raise ValueError(f"event triggered_by 非法: {event['triggered_by']}")
    if not event["title"] or not event["summary"]:
        raise ValueError("event 的 title/summary 不能为空")
    prompt = str(event.get("text_to_image_prompt", "")).strip()
    if not prompt:
        raise ValueError("event 的 text_to_image_prompt 不能为空")
...
def write_manifest(
    ...
    payload = {
        ...
        "phase_a_outputs": {
            ...
            "text_to_image_prompts_jsonl": str(text_to_image_prompts_path),
        },
        "phase_c_seedance_outputs": {
            "status": "pending_first_frame_images",
            "note": "文生图生成首帧图后，再结合首帧图和 A2 结果生成 Seedance 图生视频 prompt。",
        },
    }
...
def run_pipeline(
    ...
    events = generate_events_with_gemini(contexts, batch_size=batch_size)

    contexts_path = base_dir / "phase_a_contexts.jsonl"
    text_to_image_prompts_path = base_dir / "phase_a_text_to_image_prompts.jsonl"
    manifest_path = base_dir / "manifest.json"

    write_jsonl(contexts_path, contexts)
    write_jsonl(text_to_image_prompts_path, events)
    ...
    write_manifest(
        ...
        text_to_image_prompts_path=text_to_image_prompts_path,
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="秃秃视频生产流：A1 context + A2 文生图 prompt 批量生成器")
...
def main() -> None:
    ...
    summary = {
        ...
        "phase_a_text_to_image_prompts": str(artifacts.text_to_image_prompts_path),
