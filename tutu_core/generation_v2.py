...
· 以"图片1是小蘑菇角色形象参考。"开头（提交时系统会自动扩充为"图片1是小蘑菇角色参考。图片2是肢体末端（圆形无爪子）特写参考。图片3是张嘴表情特写参考。图片4是全身比例参考。[若有强情绪，图片5+是对应表情参考]"）
· **推荐在正文描述动作/表情时显式引用对应图片**：描写手部用"参考图片2"，描写张嘴用"参考图片3"，描写比例用"参考图片4"，描写强情绪用"参考图片5/6"
...
```json
{{
  "video_prompt": "图片1是小蘑菇角色形象参考。...（含视觉风格/光线/比例/运镜/5-6段分镜/配乐音效段/温馨收尾）",
  "inner_voice": "菇的碎碎念文案......",
