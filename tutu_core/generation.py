...
· 收尾把不完整的成果若无其事递给镜头""",
        ...
        "秃秃家中": """本事件发生在【秃秃自己家】，是蘑菇尺度的童话室内空间。要求：
· 情绪弧线：温馨开场 → 缓慢专注做某件小事 → 出现一点小惊喜或小波折 → 满足定格
· 【尺度铁律】秃秃和家具是匹配尺度，不要写"对它来说像山一样大""比身体高 N 倍""巨大的 XX"等微缩反差描写——这条**绝对禁止**
· 必须用 `{scene:KEY}` 占位符锚定场景（4 选 1：bedroom/living_bedroom/game_dressing/entrance），并在场景描述里复述参考图的关键陈设
· 装饰元素只能是自然/童话风：浆果、树叶、花瓣、草绳、萤火苔藓、蘑菇灯、唱片、风铃、星座；**禁止出现现代人类电器**（手机/电脑/微波炉/咖啡机等）
· 画面里不出现 NPC/人类——这是秃秃独处的家
· 节奏比 collection 慢，主打"独处也很满足"的氛围；身体材质 duang/啪叽/软乎 等可保留但不必每段都强调
· 收尾不强求互动 beat 翻车搞笑，温柔定格也很好（眯眼/慢慢眨眼/帽子歪着定格）""",
    }
...
def quality_review(prompt_text: str, category: str) -> tuple[bool, list[str]]:
    ...
    # 禁止说话/心理语言检查 — 双层过滤：
    #   层1：引导词 + 引号包裹（涵盖 "仿佛说""心里想""意思是""示意""告诉" 等隐性变体）
    #   层2：引号里出现人称代词 + 句尾助词，几乎一定是台词（即使没有引导词）
    speech_patterns = [
        # 层1：引导词 + "..."
        # （a）含"说/喊/叫"+引号
        r'(?:仿佛|好像|无声地|默默|心里|似乎|宛若|犹如|像)[^\n]{0,15}[说喊叫][^\n]{0,4}["“][^"”]{2,}["”]',
        r'[说喊叫]道?[:：]?\s*["“][^"”]{2,}["”]',
        r'像在说[：:]?\s*["“][^"”]{2,}["”]',
        # （b）非"说"系引导词：心里想/意思是/表示/表达/示意/告诉/想说/想表达
        r'(?:心里想|心想|意思是|意思像|意思就是|表示|表达|示意|告诉|想说|想表达|无声地表达)[^\n]{0,6}["“][^"”]{2,}["”]',
        # （c）"用 X 说/告诉/表达"
        r'用[^\n]{1,8}(?:说|告诉|表达|表示|示意)[^\n]{0,6}["“][^"”]{2,}["”]',
        # 层2：引号里含人称代词+常见结尾助词，即使没有引导词也按台词论
        # （这一层兜底 LLM 把"...嘟，仿佛"省了直接写引号台词的情形）
        r'["“][^"”]{0,15}[我你咱][^"”]{0,15}[了啦呢吗呀哦嘛哒][^"”]{0,5}["”]',
    ]
    for pat in speech_patterns:
        if re.search(pat, prompt_text):
            issues.append(
                "检测到说话/台词/心理语言旁白（禁止）：秃秃只能用拟声词。"
                "禁止任何\"仿佛在说/心里想/意思是/示意/告诉/表达\"等心理语言引号台词，"
                "也禁止引号里出现人称代词（我/你/咱）+ 句末助词 这种台词式短句。"
            )
            break
...
def generate_event_content(
    ...
    # Step 1: 分类（支持 event 字典携带 category 字段做 override，
    # 用于调用方已经知道正确类别、不希望被 classify_event 关键词扫描误判的场景；
    # 不带 category 字段时走原有 classify_event 逻辑，保持向后兼容）
    category = event.get("category") or classify_event(title, summary)
    logger.info(f"事件 [{title}] 分类为: {category}")

    # Step 2: 加载范例和指引
    # 支持 event["skip_example"]=True 跳过 few-shot 范例注入（向后兼容，默认 False）。
    # 使用场景：调用方已经在 summary 里带了非常明确的分镜蓝本+主题锁定约束，
    # few-shot 反而会干扰 LLM 忠实按蓝本展开（比如 collection_batch 场景）。
    example = "" if event.get("skip_example") else _load_example_for_category(category)
...
视频prompt硬性规则：
· **不要**在开头写任何"图片1是... 图片2是..."的图片声明（系统会在提交前自动规则注入完整声明，你写的会被覆盖删除）
· **直接从"摄影风格："这一行开始写**，依次是 摄影风格 → 光线 → 构图 → 比例 → 场景描述 → 分镜
· 固定图片编号（1-5 是固定的，你可以直接引用）：
  - 图片1=角色参考 / 图片2=肢体末端 / 图片3=张嘴 / 图片4=屁股 / 图片5=全身比例
· **表情图编号不固定（图片6+）**：当你描述强情绪时，**使用占位符**而不是直接写编号：
  - `{{happy}}` = 开心
  - `{{cry}}` = 委屈哭泣
  - `{{shy}}` = 害羞
  - `{{angry}}` = 生气奶凶
  - `{{laugh}}` = 大笑
  系统会根据你实际描写的情绪，把占位符替换成正确的"图片6/图片7…"编号并附带对应参考图。
· **推荐在正文描述动作/表情时显式引用对应图片**，例如：
  - 描写手部动作：`参考图片2的肢体末端形态`
  - 描写张嘴：`参考图片3嘴内深色的圆洞`
  - 描写背影/屁股：`参考图片4的屁股形态`
  - 描写整体比例：`参考图片5的全身比例`
  - 描写强情绪（使用占位符，别写图片编号）：`眯眼笑，腮帮子鼓鼓的（参考{{happy}}表情图）`
  - 另一种情绪：`眼泪在眼眶里打转（参考{{cry}}情绪图）`
...
· **禁止说话（硬性铁律，违反直接重写）**：秃秃不能说人话，不能有任何台词、独白、或心理语言旁白。
  - 严禁的引导词（出现即违规，不要试图绕过）：
    `说 / 喊 / 叫 / 仿佛说 / 好像说 / 像在说 / 似乎说 / 无声地说 / 默默说 /
     心里想 / 心想 / 意思是 / 表示 / 表达 / 示意 / 告诉 / 想说 / 想表达 /
     用眼神说 / 仿佛在说 / 仿佛在告诉 / 像在告诉` 等。
    **任何引号包裹的完整人类语句都不允许**，即使加了"无声地/仿佛/好像/像/意思是"等缓冲词都不行。
  - 引号里**严禁出现人称代词**："我搞定了"、"你看我"、"我做到了" 等都是违规；
    引号里只允许出现纯拟声词（如"嘟～"、"啊——呜——"、"哼哧"、"咔哒"）。
  - 只能用**拟声词**：嘟 / 嘟嘟 / 嘟～ / 嘟！ / 嘟—— / 呼噜 / 哼哧 / 哈欠（啊——呜——）/ 啪叽 / duang / 噗 / 咔 / 沙沙 / 噔噔 等
  - 情绪和想法只通过**表情+肢体动作+拟声词**传达，不通过语言内容传达
  - 反例 ❌：
      `仿佛在说"我搞定了"，最后定格`
      `点了点头，意思是"这个给你"`
      `用眼神告诉镜头"快看"`
  ...
  - 正例 ✅：
      `点头然后定格`
      `带着一点点小骄傲望向镜头，发出一声满足的"嘟～"`
      `像是在分享什么似的歪了歪头，腮帮子鼓鼓的`（描述动作姿态而非语言）
· 结尾2-3秒有互动beat（望镜头/递东西/眨眼/定格）
· 末尾必须写："只要音效，不要背景音乐，不要字幕。角色特征：小蘑菇的嘴巴张开时里面是黑色的小圆洞（无牙齿无舌头），身体两侧伸出的肢体末端是圆滚滚的肉球状/棉花糖状（整体一个圆球，无分开的指头，无爪状结构），没有眉毛，没有尾巴。"
· 总字数控制在500-700字
· 不要有吃东西的画面
· 角色外貌铁律（非常重要，描述时严格遵守用词）：
  - **不要使用"手"或"小手"这两个词**。改称"肢体末端"或"圆球状的小短肢"
  - 肢体末端是圆滚滚的肉球状（像棉花糖/小汤圆/毛球），整体是一个圆球，没有任何指头分叉，没有爪状结构
  - 嘴巴张开时里面是纯黑色的小圆洞（无牙齿、无舌头），平时只是一个小弧线
  - 身体：没有尾巴、没有眉毛
· 描述抓握/触碰动作时用："肢体末端按上去/抱住/扶着" 而不是"手抓/手指捏" """
...
```json
{{
  "video_prompt": "摄影风格：...。光线：...。构图...。比例：...。背景物体用浅景深虚化成柔和色块。0-3s... 3-7s... 7-10s... 10-13s... 只要音效...角色特征...",
  "inner_voice": "菇的碎碎念文案......",
...
```"""
        ...
        # Step 4: 质量校验（final prompt 会在 submit 时被注入图片1声明，这里先跑 LLM 原始输出）
        ...
        if passed:
            ...
            # 把通过那一轮的 LLM 输入（system + user）一并归档，供调用方落盘排查
            content["_llm_system_prompt"] = system_prompt
            content["_llm_user_prompt"] = user_prompt
            if attempt > 0:
