...
def quality_review(prompt_text: str, category: str) -> tuple[bool, list[str]]:
    ...
    # 禁止说话检查 — 任何引号包裹的长语句 + "说""仿佛说"类缓冲词都算违规
    speech_patterns = [
        r'(?:仿佛|好像|无声地|默默|心里|似乎)[^\n]{0,10}说[^\n]{0,4}["“][^"”]{3,}["”]',  # 仿佛在说"..."
        r'[说喊叫]道?[:：]\s*["“][^"”]{3,}["”]',  # 说："..."
        r'像在说[：:]?\s*["“][^"”]{3,}["”]',
    ]
    for pat in speech_patterns:
        if re.search(pat, prompt_text):
            issues.append("检测到说话/台词（禁止）：秃秃只能用拟声词，不能有任何引号包裹的人类语句或'说'字引导的对白")
            break
...
· **禁止说话（硬性铁律）**：秃秃不能说人话，不能有任何台词或独白。
  - 分镜描述中**严禁出现**：`秃秃说"..."` / `秃秃喊"..."` / `仿佛在说"..."` / `无声地说"..."` / `心里想"..."` / `像在说"..."`这类表达，**任何引号包裹的完整人类语句都不允许**（即使加了"无声地""仿佛""好像""意思是"等缓冲词）
  - 只能用**拟声词**（拟声词可以出现）：嘟 / 嘟嘟 / 嘟～ / 嘟！ / 嘟—— / 呼噜 / 哼哧 / 哈欠（啊——呜——）/ 啪叽 / duang / 噗 / 咔 / 沙沙 / 噔噔 等
  - 情绪和想法只通过**表情+肢体动作+拟声词**传达，不通过语言内容传达
  - 反例：`仿佛在无声地说："看，我搞定了。"` ❌
  - 正例：`带着一点点小骄傲望向镜头，发出一声满足的"嘟～"` ✅
...
```json
{{
  "video_prompt": "摄影风格：...。光线：...。构图...。比例：...。背景物体用浅景深虚化成柔和色块。0-3s... 3-7s... 7-10s... 10-13s... 只要音效...角色特征...（不要写图片1/图片2声明，直接从摄影风格开始）",
  "inner_voice": "菇的碎碎念文案......",
