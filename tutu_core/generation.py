...
视频prompt硬性规则：
· 以"图片1是小蘑菇角色形象参考。"开头（提交时系统会自动扩充成"图片1是小蘑菇角色参考。图片2是肢体末端（圆形无爪子）特写参考。图片3是张嘴表情特写参考。图片4是全身比例参考。[若有强情绪，图片5+是对应表情参考]"）
· **推荐在正文描述动作/表情时显式引用对应图片**，例如：
  - 描写手部动作时可写"参考图片2的肢体末端形态"
  - 描写张嘴时可写"参考图片3嘴内深色的圆洞"
  - 描写整体比例时可写"参考图片4的全身比例"
  - 描写强情绪时可写"参考图片5的开心表情"等
