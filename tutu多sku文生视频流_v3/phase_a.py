...
def build_context_generation_prompts(
    ...
    history_block = ""
    if previous_themes:
        # 取最近 150 条作为反例（兼顾上下文长度），太长容易让 Gemini 忽略
        recent = previous_themes[-150:]
        history_block = (
...
def check_action_theme_reasonableness(action_theme: str) -> str | None:
    """检测 A 模式声明 + 代用道具句式的写法矛盾。

    返回 None 表示通过；返回 str 表示违规原因。

    只做最小语法层面的检查：声明了"作为/真正的/是一名 X"等 A 模式短语，
    同时句中又出现"当作/当成/充当"等 B 模式代用道具句式——这是明显矛盾。
    其它合理性（场景与身份是否匹配等）由 prompt 约束 + Gemini 自查处理，
    不维护任何关键词清单避免硬编码偏见。
    """
    if not action_theme:
        return None
    if not A_CLAIM_PATTERN.search(action_theme):
        return None
    # 含 cosplay 词的合法 B 模式
    if any(w in action_theme for w in ["模仿", "扮演", "假装", "装作"]):
        return None
    # A 模式声明 + 代用道具句式 → 矛盾
    if PROP_SUBSTITUTE_PATTERN.search(action_theme):
        return f"A 模式声明 '{A_CLAIM_PATTERN.search(action_theme).group()}' 但出现代用道具句式（当作/当成/充当）"
    return None
...
def generate_contexts_with_gemini(blueprints: list[dict[str, Any]], batch_size: int) -> list[dict[str, Any]]:
    ...
    for batch in chunks(blueprints, batch_size):
        ...
        bad_examples: list[tuple[str, str]] = []  # (action_theme, reason)
